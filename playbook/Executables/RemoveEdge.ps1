...
switch ($Action) {
    ...
    "SpoofRegion" {
        ...
        if ($null -ne $nation) {
            [Microsoft.Win32.Registry]::SetValue($GeoKey, 'ArgusOS_SavedNation', $nation, [Microsoft.Win32.RegistryValueKind]::String)
        }
    ...
    "RestoreRegion" {
        $saved = [Microsoft.Win32.Registry]::GetValue($GeoKey, 'ArgusOS_SavedNation', $null)
        ...
        if ($null -ne $saved) {
            ...
            if ($key) { $key.DeleteValue('ArgusOS_SavedNation', $false); $key.Close() }
        }
    }
